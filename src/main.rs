...
use crate::grpc_service::{create_grpc_server, SensorHubService};
...
async fn main() {
    ...
    let sensor_config =
        load_sensor_config(&sensor_config_path).expect("Failed to load sensor config");
    ...
    let (sensors, buses) = init_all(&sensor_config, grpc_service.clone())
        .await
